...
class Subnetwork(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.layers += [Dense(config.num_hiddens[-2], config.num_hiddens[-1], activate=True)]
...
class FeedForwardModel(nn.Module):
    ...
    def forward(self, x, dw):
        ...
        z_init = Parameter(torch.zeros([1, self._dim]).uniform_(-.1, .1)).to(TH_DTYPE).cuda()

        all_one_vec = torch.ones((dw.shape[0], 1), dtype=TH_DTYPE).cuda()
        y = all_one_vec * self._y_init
        # y = self._subnetwork_value(x[:,:,0])
