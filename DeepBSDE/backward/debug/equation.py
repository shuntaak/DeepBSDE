...
class toy1d(Equation):
    ...
    def f_th(self, t, x, u, z):
        ...
        # return (torch.cos(x)*(1+self._sigma**2/2)+self._mu*torch.sin(x))*np.exp((self.total_time-t)/2)-(torch.cos(x)*torch.sin(x)*np.exp(self.total_time-t))**2/2+(u*z)**2/2
         # return (torch.cos(x)*(1+self._sigma**2/2)+self._mu*torch.sin(x))*np.exp((self.total_time-t)/2)
