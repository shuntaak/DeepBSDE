...
class FeedForwardModel(nn.Module):
    ...
    def forward(self, x, dw):
        ...
        #print('y qian', y.max())
        y = y - self._bsde.delta_t * (self._bsde.f_th(time_stamp[self._t], x[:, :, self._t], y, z))
        #print('y hou', y.max())
        ...
        # print(self._target(x[:, :, 0]))
        # if self._t ==0:
        #     print(loss)
        # print(self._target(torch.Tensor(1)))
        return loss
    ...
    def eval_loss(self,x,dw):
        ...
        del net1
        y = self._subnetwork_value(x[:, :, self._t])
        # print(torch.mean(torch.abs(y)).item())
        # print(self._subnetwork_value)
        # print(torch.mean(torch.abs(y)-torch.abs(self._subnetwork_value(x[:,:,self._t]*100))).item())
        z = self._subnetwork_grad(x[:, :, self._t]) 
        #print('y qian', y.max())
        u = y - self._bsde.delta_t * (self._bsde.f_th(time_stamp[self._t], x[:, :, self._t], y, z))
