...
class AllenCahn(Equation):
    ...
    def sample(self, num_sample):
        ...
        x_sample[:, :, 0] =  normal.rvs(size=[num_sample,
                                         self._dim, 1])
        x_sample[0, :, 0] =  np.zeros(self._dim)
...
class HJB(Equation):
    ...
    def f_th(self, t, x, u, z):
        return -self._lam*torch.norm(torch.sum((z/self._sigma)**2, dim=1, keepdim=True))
...
class HJB1(Equation):
    def __init__(self, dim, total_time, num_time_interval):
        ...
        self._x_init = np.ones(self._dim)*1.
        self._sigma = np.sqrt(2.0)
        self._lam = 1.0
...
class toy1d(Equation):
    def __init__(self, dim, total_time, num_time_interval):
        super(toy1d, self).__init__(dim, total_time, num_time_interval)
        self._x_init = 1.
        self._sigma = 1.0
        self._mu = 0.2
    ...
    def sample(self, num_sample):
        dw_sample = normal.rvs(size=[num_sample,
                                     ...
                                     self._num_time_interval]) * self._sqrt_delta_t
        x_sample = np.zeros([num_sample, self._dim, self._num_time_interval + 1])
        x_sample[:, :, 0] = np.ones([num_sample, self._dim])
        for i in range(self._num_time_interval):
            x_sample[:, :, i + 1] = x_sample[:, :, i] + self._mu * x_sample[:,:,i] * self._delta_t+ self._sigma * dw_sample[:, :, i]
        return torch.FloatTensor(dw_sample), torch.FloatTensor(x_sample)
        
    def f_th(self, t, x, u, z):
        # return (torch.cos(x)*(np.exp((self.total_time-t)/2)+self._sigma**2/2)+self._mu*torch.sin(x))*np.exp((self.total_time-t)/2)-(torch.cos(x)*torch.sin(x)*np.exp(self.total_time-t))**2/2+(u*z)**2/2
        return (torch.cos(x)*(1/2+self._sigma**2/2)+self._mu*torch.sin(x))*np.exp((self.total_time-t)/2)-(torch.cos(x)*torch.sin(x)*np.exp(self.total_time-t))**2/2+(u*z)**2/2
#          return (torch.cos(x)*(1/2+self._sigma**2/2)+self._mu*torch.sin(x))*np.exp((self.total_time-t)/2)
